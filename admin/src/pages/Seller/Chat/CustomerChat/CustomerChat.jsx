...
const CustomerChat = () => {
    ...
    useEffect(() => {
        if (success_message) {
            socket.emit(
                "send_message_seller_to_customer",
                customer_seller_messages[customer_seller_messages.length - 1]
            );
    ...
    useEffect(() => {
        if (receiveMessage) {
            if (
                customerId === receiveMessage.senderId &&
                user_info._id === receiveMessage.receiverId
            ) {
                dispatch(update_message_customer(receiveMessage));
                notification();
            } else {
    ...
    }, [customerId, dispatch, notification, receiveMessage, user_info._id]);
        ...
        <div className="px-2 lg:px-7">
            <div className="w-full bg-[#dae1e7] px-4 py-4 rounded-md">
                <div className="flex w-full h-full relative">
                    <div
                        className={`w-[250px] h-full z-10 absolute ${
                        ...
                        <div className="w-full h-[calc(100vh-177px)] bg-[#252b3b] md:bg-transparent overflow-y-auto">
                            ...
                            {customers.map((c, index) => (
                                <Link
                                    ...
                                    key={index}
                                    className={`h-[50px] flex justify-start gap-2 items-center mt-3 px-2 py-2 rounded-xl cursor-pointer ${
                                        customerId === c?.friendId ? "bg-slate-700 text-white" : ""
                                    }`}
                                >
                                    ...
                                    <div className="flex justify-center items-start flex-col w-full">
                                        <div className="flex justify-between items-center w-full">
                                            <h2 className="text-base">{c.customer_name}</h2>
                                        </div>
                                    </div>
                                </Link>
                            ))}
                    ...
                    <div className="w-full md:w-[calc(100%-230px)] md:pl-4">
                                                            ...
                                                            {m.file === null ? (
                                                                ...
                                                                <>
                                                                    ...
                                                                    {(() => {
                                                                        if (m && m.file) {
                                                                            const url = m.file;
                                                                            const fileName = url.split("?")[0]; // remove parameters
                                ...
                                <div className="flex p-2 justify-between items-center w-full">
                                    ...
                                    <div className="w-[calc(100%-150px)] rounded-full overflow-visible">
                                        <input
                                            ...
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                    e.preventDefault(); // Ngăn việc Enter thêm dòng mới
                                                    handleSendMesaage(); // Gọi hàm gửi tin nhắn
                                                }
                                            }}
                                        />
                                    </div>
