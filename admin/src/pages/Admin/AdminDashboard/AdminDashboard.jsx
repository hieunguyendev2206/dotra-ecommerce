...
import {get_admin_dashboard_data, get_data_on_chart,} from "../../../store/reducers/dashboard.reducers";
...
const AdminDashboard = () => {
    const {
        AiOutlineShoppingCart, FaMoneyCheckDollar, FaProductHunt, HiOutlineUserGroup,
    } = icons;
    ...
    function translateStatusToVietnamese(status) {
        const statusTranslations = {
            processing: "Đang xử lý", shipping: "Đang vận chuyển", delivered: "Đã giao", canceled: "Đã hủy",
        };
    ...
    const state = {
        series: [{
            name: "Đơn hàng", data: monthlyOrderCounts,
        }, {
            name: "Sản phẩm", data: monthlyProductCounts,
        }, {
            name: "Danh mục", data: monthlyCategoryCounts,
        }, {
            name: "Người bán", data: monthlySellerCounts,
        }, {
            name: "Khách hàng", data: monthlyCustomerCounts,
        },], options: {
            title: {
                text: "BIỂU ĐỒ THỐNG KÊ", align: "center", margin: 10, offsetX: 0, offsetY: 0, floating: false, style: {
                    fontSize: "14px", fontWeight: "bold", color: "#263238",
                },
    ...
    const line = {
        series: [{
            name: "Doanh thu", data: monthlyRevenues, color: "#FF0000",
        },], options: {
            title: {
    ...
    const donut = {
        series: percentagesArray.map((p) => p.percentage), options: {
            chart: {
                type: "donut",
            },
            labels: percentagesArray.map((p) => translateStatusToVietnamese(p.status)),
            colors: ["#FFFF00", "#0000FF", "#00FF00", "#FF0000"],
            title: {
                text: "BIỂU ĐỒ THỐNG KÊ TỶ LỆ ĐẶT HÀNG", align: "center", style: {
                    fontSize: "14px", fontWeight: "bold", color: "#263238",
                },
            },
            responsive: [{
        ...
        <div className="overflow-x-auto mt-10">
            <Table hoverable>
                ...
                <Table.Body className="divide-y">
                    {recent_order && recent_order.map((o) => (<Table.Row
                        key={o._id}
                        className="bg-white dark:border-gray-700 dark:bg-gray-800"
                    >
                        <Table.Cell>#{o._id.slice(0, 5)}...{o._id.slice(-5)}</Table.Cell>
                        ...
                        <Table.Cell>
                            <div className="flex flex-warp gap-4">
                                <Link to={`/admin/dashboard/orders/details/${o._id}`}>
                                    <button
                                        className="flex i   tems-center justify-center bg-[#f1f1f1] dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg p-2">
